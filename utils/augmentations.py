...
class RandomTranslate(object):
    ...
    def __call__(self, imgs, mask):
        ...
        for (idx, img) in enumerate(imgs):
            ...
            img = tf.pad(cropped_img, padding_tuple, padding_mode="constant",fill=[0,0,0])
            mask_ = tf.affine(
                    mask,
...
class RandomBiasField():
    def __init__(self,coefficients_range=(0,0.5),order=2) -> None:
        random_coefficients = []
        ...
        self.transform = tio.BiasField(coefficients=random_coefficients,order=order)
...
class RandomNoise():
    def __init__(self,mean_range=1.0,std_range=0.03) -> None:
        mean = self.sample_uniform(*mean_range)
        std = self.sample_uniform(*std_range)
        seed = self._get_random_seed()
        self.transform = tio.Noise(mean=mean,std=std,seed=seed)
